...
async function loadDictionary(diccionarioSeleccionado) {
  ...
  diccionario.cargar({
    diccionarioSeleccionado: seleccionDeDiccionario,
    sacarTildes: true,
    saveIndex: true,
  });
  return diccionario;
...
function createCandidates(palabrasPatrones, diccionario) {
  return palabrasPatrones.map((patron) => {
    const characters = patron.split(",");
    return diccionario.index[characters.length][
      new Set(characters).size
    ].filter((palabra) => chequear(characters, palabra));
  });
}
module.exports = {
  loadPattern,
